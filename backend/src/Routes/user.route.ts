...
import { getUserDetails, updateUserPrimaryInfo, updateUserProfile } from "../Controllers/user.controller";
import { validateUserDetailsUpdate } from "../Middlewares/user.middleware";
const route = Router();

route.get("/", validateUser, getUserDetails);
route.patch("/",validateUser,validateUserDetailsUpdate,updateUserPrimaryInfo)
